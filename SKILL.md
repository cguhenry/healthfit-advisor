...
7. 若需求是「外食怎麼選」「超商/自助餐/餐廳建議」「手搖飲/早餐店/便當店/滷味/火鍋/麵店」，優先用：
   - `scripts/healthfit.py dining`
   - `scripts/diet_dialogue.py`（多輪對話引導）

   若只是一般菜單靈感且不需要今日狀態，可保留 `scripts/menu_advisor.py` 作為 legacy fallback。
...
- Unified CLI now supports one-shot Phase 3 → Phase 4 ingestion via `scripts/healthfit.py log from-image`, which parses the raw image-analysis JSON and writes it into the calorie tracker directly.
- Agent/skill image intake now also has a formal handoff helper via `scripts/healthfit.py image prompt`: when a meal photo arrives, build the multimodal prompt bundle, save the model's JSON-only reply as `phase3_response.json`, then call `log from-image` (optionally via stdin + `--save-raw-response`) for near one-shot logging.
- Daily check-in Q&A now has a formal handoff too: `scripts/healthfit.py checkin prompt` builds the question, and `scripts/healthfit.py checkin answer` parses the user's natural-language meal reply, converts it into manual meal rows, writes `food_logs`, and refreshes the daily summary.
- Fixed-time daily check-in is now formalised too: `scripts/healthfit.py notify checkin` generates/delivers the meal question, and `notify setup-cron` now includes a lunch check-in entry in addition to the daily/weekly reports.
- Plateau alerts now trigger action, not just warning: on a loss plan, a confirmed 2-week plateau auto-runs the dynamic solver, then either lowers calories by 100 kcal or recommends one extra exercise day, and writes a new active plan into `weight_plans`.
...
- `scripts/diet_dialogue.py`
  - Phase 2 對話引導樹，解析 cuisine/location/meal 自然語言，支援多輪 state 持續；也提供每日 check-in 的自然語言餐次解析與手動落庫膠水層
...
- `scripts/health_alerts.py`
  - Phase 6 健康警示系統，偵測 7 種徵兆：低熱量連續日、快速減重、蛋白質不足、長期未記錄、體重停滯、暴食日、運動過量；停滯達 2 週時會自動重算並寫入新的 active plan
- `scripts/gi_guide.py`
...
- `scripts/notification_scheduler.py`
  - Phase 6 Cron 排程報告與每日 check-in 提問，支援 Discord/LINE webhook 傳送
- `scripts/food_db_cache.py`
  - Phase 7 食品資料庫快取（TTL in-memory cache，無 Redis 依賴）
- `scripts/food_db_lookup.py`
  - Phase 7 食品查詢引擎，模糊搜尋 TW_FDA + USDA 資料庫
  - **Source Priority**：中文 query → TW_FDA 優先，英文 query → USDA 優先
- `scripts/bootstrap_food_db.py`
  - 正式食品營養資料庫一次性啟動匯入（TW_FDA + USDA）。**正式匯入請用這個，不要用 `food_db_importer.py`**。
  - 用法：`python scripts/bootstrap_food_db.py`
  - 或分別執行：`python scripts/food_db_import_pivot.py import-all --usda-dir assets/usda_food_db/foundation_foods_csv`

## Food DB Import（正式資料庫匯入）

**千萬不要用 `food_db_importer.py` 匯入 TW_FDA 或 USDA 正式資料。**

| 資料來源 | 格式 | 正確匯入方式 |
|---|---|---|
| TW_FDA (`assets/tw_food_db/tw_food_db.csv`) | 一食品多列 pivot | `python scripts/food_db_import_pivot.py import-tw` |
| USDA Foundation Foods | 多檔關聯表（food.csv + food_nutrient.csv）| `python scripts/food_db_import_pivot.py import-usda --usda-dir assets/usda_food_db/foundation_foods_csv` |
| 一行一食品簡單 CSV | 扁平格式 | `python scripts/food_db_importer.py import-tw --file YOUR_FILE.csv` |
| 一次性匯入（TW + USDA）| — | `python scripts/bootstrap_food_db.py` |

執行後可用 `python scripts/food_db_status.py` 確認匯入結果。
- `scripts/privacy_manager.py`
  - Phase 7 隱私管理，匯出/刪除個人資料（HIPAA/GDPR 基礎）
- `scripts/can_i_eat.py`
  - Feature F：「今天能不能吃 X？」即時熱量評估。串接 get_calorie_progress()（今日進度）、FoodDBLookup.search()（熱量查詢）與 verdict 判斷邏輯，支援份量倍數（--quantity）與餐別提示（--meal）。若食物不在資料庫中，自動使用預設份量估算並標記為低信心度。
  - **注意**：執行前需有 active weight plan；無 plan 時會明確拋出 RuntimeError。
...
- 在 Henry 的直接聊天情境中，若訊息以 `早餐` / `午餐` / `晚餐` / `點心` 開頭，且後面直接接自由文字餐點內容，視為「記錄攝取」意圖，不要求使用者額外輸入「使用healthfit-advisor」等完整指令。
- 若訊息格式為 `M/D 餐別 內容`（例如 `6/13 晚餐 大碗酢醬乾麵1碗，燙青菜半盤`），優先解析日期與餐別後直接走 `checkin answer`。
- 若訊息只有 `餐別 + 內容`，未附日期時，預設以 `Asia/Taipei` 的今天作為記錄日期。
- 保留完整長句觸發方式；簡短餐別開頭格式只是額外捷徑，不是唯一入口。
...
| 觸發關鍵字 | 模組 |
|---|---|
| 減重/增肌/熱量/TDEE/BMR | `intake_flow.py` → `bwp_calculator.py` |
| 今天吃了什麼/記錄攝取/早餐.../午餐.../晚餐.../點心.../M/D 早餐... | `checkin answer` → `diet_dialogue.py` → `calorie_tracker.py` |
| 要吃什麼/外食建議/超商吃什麼 | `dining_advisor.py` / `diet_dialogue.py` / `healthfit.py dining` |
| 今天還能吃/能不能吃/吃 X 會不會超標/可以喝/還有多少熱量可以吃/這樣吃會不會太多 | `can_i_eat.py` |
| GI/升糖指數/低 GI 建議 | `gi_guide.py` |
| 熱量/蛋白質/歷史對比 | `calorie_tracker.py` |
| 偏好/最愛/避免 | `food_preference_engine.py` |
