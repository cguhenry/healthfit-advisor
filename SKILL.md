...
- `daily_summaries` are recalculated from scratch each call (no incremental aggregation).
...
- `scripts/calorie_tracker.py`
  - Phase 4 熱量追蹤模組，將 Phase 3 分析結果寫入 food_logs、自動彙總 daily_summaries、提供歷史對比（vs 昨日/上週/7天平均/計劃起始日）與滾動趨勢
...
- GI database covers 75 common Taiwanese foods; foods outside this range produce generic advice.
- Weekly meal plans are template-based, not algorithmically optimised for macro targets.
- Health alerts run on-demand or via cron; real-time push requires the notification delivery layer to be configured.
- Discord delivery uses webhooks; LINE delivery uses Messaging API. Both require external credentials configured as environment variables.
